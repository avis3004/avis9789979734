...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<ul >
		<li id="WardNumber" >
		<label class="description" for="WardElement">Ward Number </label>
		<div>
		<select class="element select medium" id="WardElement" name="WardElement"> 
			<option value="" selected="selected"></option>
		...
		</div><p class="guidelines" id="WardGuide"><small>Select your Ward Number</small></p> 
		</li>
		...
		<ul >
		<li id="StreetNameList" >
		<label class="description" for="StreetLabel">StreetName </label>
		<div>
		<select class="element select medium" id="StreetName" name="StreetName">
		</select>
		</div><p class="guidelines" id="StreetNameGuide"><small>Select your Street Name</small></p> 
		</li>
			
		<script type="text/javascript">
		ErrorFlag=0;

		document.getElementById("WardElement").onchange = function() {
		var xmlhttp;
		var select = document.getElementById("StreetName");
		var WardObj=document.getElementById("WardElement");
		var WardNumber = WardObj.options[WardObj.selectedIndex].text;
		...
		var RecordsetArray = jQuery.parseJSON(AjaxResponse);
		var ResponseSize = RecordsetArray.length;
		
			while (ResponseCounter < ResponseSize)
			{
				StreetName = RecordsetArray[ResponseCounter].StreetName;		
				var options = document.createElement("option");
				options.text = StreetName;
				options.value = StreetName;
				select.appendChild(options);
				/*document.getElementById("StreetName").text = StreetName;
		...
		xmlhttp.open("GET","/avis9789979734/finalmudichurpanchayat/MudichurPanchayat/WardResponse.php?WardNumber="+encodeURIComponent(WardNumber),true);
		xmlhttp.send();
		}
		};
		</script>
